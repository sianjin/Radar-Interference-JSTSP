...
fr1 = 3.11; % IF frequency: 8.11MHz, 5.11MHz, 3.11MHz, ghost
fr2 = 3.12; % IF frequency: 8.12MHz, 5.12MHz, 3.12MHz, target   or  7.05
...
% 2nd ghost
% d_g2 = 400;
% ghostPowerLinear2 = SignalPower0 * (c*10^6)^2/(4*pi)^2/(fc*10^6)^2/ (d_g2)^2; % ghost
% ghostAmpLinear2 = sqrt(ghostPowerLinear2);
%% unslotted ALOHA with phase coding
% example: 1 ghost and 1 target
code01 = binornd(1,2.5/5,1,L);
PC1 = exp(i*pi*code01);
X_2D = [];
for l = 1: L
    X_2D = [X_2D; ghostAmpLinear*exp(i*2*pi*fr1*t_r)*exp(i*2*pi*fD1*t_l(l)/1000)*PC1(l) + targetAmpLinear*exp(i*2*pi*fr2*t_r)*exp(i*2*pi*fD2*t_l(l)/1000)];
...
% example: 2 ghosts 
% code01 = binornd(1,2.5/5,1,L);
% code01_p = binornd(1,2.5/5,1,L);
% PC1 = exp(i*pi*code01);
% PC2 = exp(i*pi*code01_p);
% X_2D = [];
% for l = 1: L
%     X_2D = [X_2D; ghostAmpLinear*exp(i*2*pi*fr1*t_r)*exp(i*2*pi*fD1*t_l(l)/1000)*PC1(l) + ghostAmpLinear2*exp(i*2*pi*fr2*t_r)*exp(i*2*pi*fD2*t_l(l)/1000)*PC2(l)];
%     % X_2D = [X_2D;  targetAmpLinear*exp(i*2*pi*fr2*t_r)*exp(i*2*pi*fD2*t_l(l)/1000)];
%     %X_2D = [X_2D; ghostAmpLinear*exp(i*2*pi*fr1*t_r)*exp(i*2*pi*fD1*t_l(l)/1000)*FH1(l)];
% end
%% Range FFT
...
Power_Y_r_2D = 10*log10(abs(Y_r_2D).^2); % detect
countY_r_2D = Power_Y_r_2D > -128;
classifyY_r_2D = sum(countY_r_2D); % classify, entry n means number of signals over L chirp cycles on range bin n
%% Doppler FFT
Y_rD_2D = [];
for n = 1: N
...
mesh(Range, Velocity, Pow_Y_rD_2D_dB) % 2D-FFT result
colorbar
colormap(hot)
cmap = colormap;
cmapNew = flip(cmap);
...
ylabel('Velocity (m/s)')
% conclusion: 
% 1. when ghost and target are not in the different range bins, phase coding spread ghost into noise, using CFAR detector, only target
% can be detected
% 2. when ghost and target are not in the same range bin, and when ghost
% power is large (e.g., interfer range <= 300m) and target power is small (e.g., target fr = 8MHz), CFAR detector cannot detect target