...
fD1 = 4.20; % Doppler: 4.21kHz -> ghost 16.40m/s  
...
%% pure ALOHA 
% example 1: 1 ghost and 1 target
X_2D = [];
for l = 1: L
    X_2D = [X_2D; ghostAmpLinear*exp(i*2*pi*fr1*t_r)*exp(i*2*pi*fD1*t_l(l)/1000) + targetAmpLinear*exp(i*2*pi*fr2*t_r)*exp(i*2*pi*fD2*t_l(l)/1000)];
end
%% Range FFT
Y_r_2D  = [];
for l = 1: L
