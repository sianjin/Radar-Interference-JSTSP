...
X_2D = [];
for l = 1: L
    X_2D = [X_2D; ghostAmpLinear*exp(i*2*pi*fr1*t_r)*exp(i*2*pi*fD1*t_l(l)/1000)*FH1(l)*PC1(l) + targetAmpLinear*exp(i*2*pi*fr2*t_r)*exp(i*2*pi*fD2*t_l(l)/1000)];
...
% ------------------------------------------------------------------------
% MATLAB 1D-CFAR 
% detector = phased.CFARDetector('NumTrainingCells',20,'NumGuardCells',2, ...
%     'ProbabilityFalseAlarm',0.001,'OutputFormat','Detection index');
% cutidx = 1:L;
% for n = 1: N
...
xlabel('Range (m)')
ylabel('Velocity (m/s)')
% Conclusion: when ghost power is large and target power is small, this
% method still can detect target without false alarm