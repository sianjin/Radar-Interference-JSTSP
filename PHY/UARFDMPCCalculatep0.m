...
for simuTimes = 1: MonteNum
    ...
    for l = 1: L
        ...
        signal_l = signal_l + ghostAmpLinear*exp(i*2*pi*fr1*t_r)*exp(i*2*pi*fD1*t_l(l)/1000)*PC1(l);
        signal_l = signal_l + ghostAmpLinear*exp(i*2*pi*fr3*t_r)*exp(i*2*pi*fD3*t_l(l)/1000)*PC3(l);
        signal_l = signal_l + ghostAmpLinear*exp(i*2*pi*fr4*t_r)*exp(i*2*pi*fD4*t_l(l)/1000)*PC4(l);
        signal_l = signal_l + ghostAmpLinear*exp(i*2*pi*fr5*t_r)*exp(i*2*pi*fD5*t_l(l)/1000)*PC5(l);
        X_2D = [X_2D; signal_l];
